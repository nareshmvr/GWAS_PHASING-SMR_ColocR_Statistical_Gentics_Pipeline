...
* `run_plink_to_vcf_conversion.sh`: Converts PLINK binary formats to VCF.
* `run_phase_shapeit4.sh`: Performs haplotype phasing using SHAPEIT4.
* `run_impute.sh`: Runs imputation steps.
...
## 3. Plots
Located in `3.Plots/`
* `topr_manhattan_coloc_map.r`: Generates publication-ready Manhattan plots and regional colocalization maps using `topr`.
